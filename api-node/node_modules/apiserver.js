...
ApiServer.prototype = {
	...
	this._database.update({ collection: 'rounds',
		    criteria: { round_id: args.round_id },
	...
	// TODO(tracy): Change this back to 2 mins from 20 mins
	every_two_minutes = Math.floor(new Date().getTime() / 1200000);
	this._database.update({ collection: 'rounds',
		    criteria: { stretch_id: args.stretch_id, loc: { $near : [lg, lt]}, finished: false, quantam: every_two_minutes},
		    doc: { "$set": { stretch_id: args.stretch_id, loc: [lg, lt], finished: false, quantam: every_two_minutes },
	...
	this._database.update({ collection: 'rounds', 
			      criteria: { stretch_id: args.stretch_id, loc: { $near: [lg, lt]}, finished: false, quantam: every_two_minutes},
			      doc: { "$set": {stretch_id: args.stretch_id, quantam: every_two_minutes, finished: false, loc: [lg, lt]}, "$inc": {users: 1}},
			      options: { upsert: true}}, function(error, docs) {
				  if (error != null) {
				      server._respond(request, response, { error: 'Finding/creating round failed.', status: 500, msg: error});
				  } else {
				      server._respondwrap(request, response, docs);
				  }
			      });
	*/
		...
		if (results === undefined || results.length === 0) {
		    ...
		    server._database.insert({ collection: 'rounds', docs: [round]}, function(results) {
				...
				server._respondwrap(request, response, round);
			    } else {
				// Shouldn't happen!
